...
import net.shoreline.client.irc.packet.client.CPacketPing;
...
public final class IRCManager extends LoggingFeature
{
    ...
    public void addToChat(String message)
    {
        mc.send(() ->
        {
            ...
            {
                sendChatMessage("§s[IRC]§7 " + message);
            }
        });
    }
    ...
    public void addServerMessageToChat(String message)
    {
        mc.send(() ->
        {
            if (mc.player != null && mc.world != null)
            {
                sendChatMessage("§s[IRC/Server]§7 "+  message);
            }
        });
    }
