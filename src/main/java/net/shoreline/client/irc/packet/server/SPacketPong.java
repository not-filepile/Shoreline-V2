package net.shoreline.client.irc.packet.server;
...
import net.shoreline.client.irc.packet.ServerPacket;
...
public final class SPacketPong extends ServerPacket
{
    ...
    private SPacketPong(String packet) throws Throwable
    {
        ...
        for (JsonElement user : activeUsers.asList())
        {
            ...
            CapeType capes = switch (capeColor.toLowerCase())
            {
                case "black" -> CapeType.BLACK;
                case "white" -> CapeType.WHITE;
        ...
        for (JsonElement user : users.asList())
        {
            ...
            OnlineUser onlineUser = new OnlineUser(userName, type, CapeType.OFF);
